...
- **1 Sep 2026:** Task approval workflow (`Staff Docs/approval_mockup.html`) — optional
  ...
  permission model in `lib/tasks/permissions.ts` (migration `0015_task_approval.sql`). **Fully
  removed 2 Sep 2026** as part of the Owner/Assigned-To ownership rebuild (Section 14 Phase
  2) — historical only, see Section 8.3's "Approval workflow" note.
- **2 Sep 2026:** Threaded task comments (`Staff Docs/further_recs_mockup.html` #1) — new
  ...
  `new-comment` notification type. See Section 8.3. **`TaskCommentsDialog` itself was later
  absorbed into the task detail panel (Section 14 Phase 3)** — the table, notification type,
  and comments feature all live on, just not as a standalone dialog/button.
...
- Task add/edit/delete (admin only, Section 8.3 — creation via `TaskFormDialog`, editing and
  deleting via the task detail panel since 2 Sep 2026) and search + sortable columns on
...
- Task activity log (Section 8.3) — new `task_activity` table, one row per changed field.
  Originally viewable via a standalone "History" button on every task row (all roles); since
  2 Sep 2026 (Section 14 Phase 3) that button is gone, absorbed into the task detail panel's
  "Activity History" section instead — still viewable by all roles.
...
**✅ All 7 phases below are done as of 2 Sep 2026 — code committed to `master`, not yet
pushed to origin.** This was a large body of work — comparable to or larger than the entire
Leads Kanban build — planned as 7 ordered phases (decisions below were confirmed with
Abdullah and are load-bearing history; each phase's own "✅ done" line names its specific
migration and whether that migration has been confirmed applied). **Migrations
`0023`-`0027` (Phases 1, 2, 3, 4, 5) still need manual application via the Supabase SQL editor**
— none of the five have been confirmed applied as of this write-up, unlike the fully-confirmed
migrations from the 1-2 Sep mockup work above. Phase 6 needed no migration. If a future
session picks this up, the immediate next step is getting `0023`-`0027` applied, then
verifying each phase's feature against live data — nothing here is blocked on further design
or clarification.
...
- A permission bug reported for task status editing (owners editing others' tasks) could not
  be reproduced as of 29 Aug 2026 — the PATCH route and RLS both correctly scoped `owner`-role
  edits to tasks the user was `assigned_to`/`co_assigned_to` on (both column names since
  renamed/removed by the 2 Sep 2026 ownership rebuild — Section 14 Phase 2 — to
  `owner_id`/`assigned_to_id`, with the same scoping logic carried forward in
  `lib/tasks/permissions.ts`). Revisit with specific repro steps if it recurs.
